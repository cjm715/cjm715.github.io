...
\begin{circuitikz}[american]
  ...
  \node at ($(chipNW)!0.5!(chipSE)+(0,-0.6)$) [font=\scriptsize]
      {$W = \dfrac{(X_1-X_2)(Y_1-Y_2)}{10\,\mathrm{V}}$};
  ...
  \node at ($(Zin)+(0.25,0)$) [right, font=\scriptsize] {$Z$};

  % Output pin (right edge).
  \coordinate (W) at (\chw, \chh-1.5);
  \node at ($(W)+(-0.25,0)$) [left, font=\scriptsize] {$W$};

  % Wiring to produce W = -XZ/10V:
  %   X1 = X, X2 = ground   (so X1-X2 = X)
  %   Y1 = ground, Y2 = Z   (so Y1-Y2 = -Z)
  %   Zin grounded
  %   --> W = X * (-Z) / 10V = -XZ/10V
  \draw (X1) -- ++(-1.6,0) node[circ]{} node[left] {$X$};
  \draw (X2) -- ++(-0.8,0) node[ground, rotate=-90, yshift=-0.05cm]{};
  \draw (Y1) -- ++(-0.8,0) node[ground, rotate=-90, yshift=-0.05cm]{};
  \draw (Y2) -- ++(-1.6,0) node[circ]{} node[left] {$Z$};
  \draw (Zin) -- ++(-0.8,0) node[ground, rotate=-90, yshift=-0.05cm]{};

  % Output wire.
  \draw (W) -- ++(1.6,0) node[circ]{} node[right] {$-\dfrac{XZ}{10\,\mathrm{V}}$};
